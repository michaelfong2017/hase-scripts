...
def main():
    ...
    # Load CSV data
    csv_file = "Dataset_Source_v5.csv"
    if len(sys.argv) > 1:
        csv_file = sys.argv[1]
        ...
        for future in as_completed(future_to_task):
            try:
                ...
                if completed_tasks % 20 == 0:  # Save every 20 completions
                    with df_lock:
                        output_file = f"dataset/t_{TEMPERATURE}_openrouter_output_{os.path.basename(csv_file)}"
    ...
    # Save final results
    output_file = f"dataset/openrouter_output_{os.path.basename(csv_file)}"
    os.makedirs("dataset", exist_ok=True)
    df.to_csv(output_file, index=False)
