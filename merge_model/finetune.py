    ...
    def log_training_step(self, step, loss, lr, step_duration):
        ...
        if loss is not None and not (isinstance(loss, float) and (loss != loss)):  # Check for NaN
            ...
            if float(loss) < self.best_loss:
                ...
                self.best_step = step
                print(f"🎯 NEW BEST LOSS: {self.best_loss:.6f} at step {step}")
                return True
        return False
...
def print_metrics_report(metrics_summary, config):
    ...
    if perf['total_steps'] > 0:
        ...
        print(f"🎯 Best Loss: {perf['best_loss']:.4f} (step {perf['best_step']})")
        print(f"📉 Total Loss Improvement: {perf['loss_improvement']:.4f}")
        
        if perf['best_loss'] < perf['final_loss']:
            improvement = perf['final_loss'] - perf['best_loss']
            improvement_pct = (improvement / perf['final_loss']) * 100
            print(f"🔥 Best model is {improvement:.4f} ({improvement_pct:.2f}%) better than final model")
        else:
        ...
        if perf['initial_loss'] > 0:
            ...
            best_improvement_pct = ((perf['initial_loss'] - perf['best_loss']) / perf['initial_loss']) * 100
            print(f"📊 Final Loss Improvement: {improvement_pct:.2f}%")
            print(f"📊 Best Loss Improvement: {best_improvement_pct:.2f}%")
    else:
...
def validate_config(config):
    ...
    if not os.path.exists(config["data"]["train_file"]):
        raise FileNotFoundError(f"Training file not found: {config['data']['train_file']}")
    if not os.path.exists(config["data"]["eval_file"]):
        raise FileNotFoundError(f"Evaluation file not found: {config['data']['eval_file']}")
...
def print_config(config):
    ...
    print(f"  Evaluation data: {config['data']['eval_file']}")
...
def main():
    ...
    try:
        ...
        if config["output"]["output_dir"] is None:
            ...
            output_model_path = f"./output_models/job_{timestamp}_{model_family}_{model_type}"
        else:
        ...
        class CustomDataset(Dataset):
            def __init__(self, file_path, tokenizer, model_family, max_seq_length):
                self.tokenizer = tokenizer
                self.max_seq_length = max_seq_length
                self.model_family = model_family

                self.is_yaml = file_path.lower().endswith('.yaml') or file_path.lower().endswith('.yml')
                self.data = []
                ...
                if self.is_yaml:
                    ...
                    filtered = [
                        entry for entry in yaml_data
                        # if entry.get("language", "").lower() == "en"
                    ]
                    print(f"📋 {len(filtered)} samples found in YAML.")
                    ...
                    print(f"📊 Loaded {len(self.data)} examples from {file_path} (YAML)")
                else:
                    print(f"📒 Parsing CSV file: {file_path}")
                    df = pd.read_csv(file_path)
                    # if 'language' in df.columns:
                    #     df = df[df['language'].str.lower() == "en"]
                    #     print(f"📋 {len(df)} samples found in CSV.")
                    self.data = [
                        {
                            "Instruction": str(row['Instruction']) if 'Instruction' in row else "",
                            "Input": str(row['Input']) if 'Input' in row else "",
                            "Output": str(row['Output']) if 'Output' in row else ""
                        }
                        for _, row in df.iterrows()
                    ]
                    print(f"📊 Loaded {len(self.data)} examples from {file_path} (CSV)")
        ...
        train_dataset = CustomDataset(train_file, tokenizer, model_family, config["training"]["max_seq_length"])
        eval_dataset = CustomDataset(eval_file, tokenizer, model_family, config["training"]["max_seq_length"])
        ...
        model = AutoModelForCausalLM.from_pretrained(
            ...
            max_memory={i: "50GB" for i in range(num_gpus)},  # Reduced from 12GB to 8GB
        )
        ...
        if use_lora:
            ...
            print(f"✓ Modules for {model_family}:")
	...
	# ==== 8. Training Loop ====
        ...
        best_model_path = os.path.join(output_model_path, "best_model")
        ...
        for epoch in range(config["training"]["num_epochs"]):
            ...
            for batch_idx, batch in enumerate(train_dataloader):
                try:
                    ...
                    if should_step:
                        ...
                        # Log to metrics tracker and check if this is the best model
                        is_best = metrics_tracker.log_training_step(global_step, avg_accumulated_loss, current_lr, step_duration)
                        ...
                        if is_best:
                            print(f"💾 🎯 Saving BEST model at step {global_step}")
                            try:
                                model.save_pretrained(best_model_path, safe_serialization=True)
                                tokenizer.save_pretrained(best_model_path)
                                
                                best_model_info = {
                                    "best_loss": float(avg_accumulated_loss),
                                ...
                                print(f"❌ Failed to save best model: {save_error}")
            ...
            # Epoch summary
            metrics_tracker.best_epoch = epoch + 1 if metrics_tracker.best_step > (epoch * (len(train_dataloader) // config['training']['gradient_accumulation_steps'])) else metrics_tracker.best_epoch
        ...
        final_model_path = os.path.join(output_model_path, "final_model")
        print("💾 Saving final model...")
        model.save_pretrained(final_model_path, safe_serialization=True, max_shard_size="5GB")
        tokenizer.save_pretrained(final_model_path)
        
        best_model_files = [
            os.path.join(best_model_path, "pytorch_model.bin"),
            os.path.join(best_model_path, "model.safetensors"),
            os.path.join(best_model_path, "adapter_model.bin"),  # LoRA adapter
            os.path.join(best_model_path, "adapter_model.safetensors")  # LoRA adapter
        ]
        
        if any(os.path.exists(f) for f in best_model_files):
            print(f"✅ Best model available at: {best_model_path}")
            print(f"   Best loss: {metrics_tracker.best_loss:.6f}")
            print(f"   Best step: {metrics_tracker.best_step}")
            
            import shutil
            main_output_path = os.path.join(output_model_path, "model")
            if os.path.exists(main_output_path):
                shutil.rmtree(main_output_path)
            shutil.copytree(best_model_path, main_output_path)
            print(f"✅ Best model copied to main output: {main_output_path}")
        else:
            print("⚠️ No best model saved, using final model")
            import shutil
            main_output_path = os.path.join(output_model_path, "model")
            if os.path.exists(main_output_path):
                shutil.rmtree(main_output_path)
            shutil.copytree(final_model_path, main_output_path)
