...
def combine_case_transactions():
    ...
    case_file_pattern = re.compile(r'Case_([A-E])_.*_transaction_record\.csv')
    case_files = {}
    for file in input_dir.glob('Case_*_*_transaction_record.csv'):
        match = case_file_pattern.match(file.name)
        if match:
            case_letter = match.group(1)
            case_files.setdefault(case_letter, []).append(file)

    removal_log = []
    before_count = 0
    emptied_counterparty_log = []

    for case_letter, files in case_files.items():
        print(f"\nProcessing Case {case_letter} with {len(files)} files:")
        dfs = []
        fraud_col_used = None
        ...
        for file in files:
            ...
            # Check for fraud payment column but don't require it
            fraud_col = find_fraud_payment_column(df.columns)
            if fraud_col:
                fraud_col_used = fraud_col
                print(f"    Found fraud payment column: {fraud_col}")
            else:
                print(f"    No fraud payment column found - proceeding anyway")
        ...
        if dfs:
            ...
            # Only sort by fraud payment if the column exists
            if fraud_col_used and fraud_col_used in combined_df.columns:
                ...
                print(f"    Sorted by {fraud_col_used}")
            else:
                print(f"    No sorting applied (no fraud payment column)")
            
            output_file = output_dir / f"Case_{case_letter}_transaction_records.csv"
            combined_df.to_csv(output_file, index=False)
            print(f"  ✅ Created output for Case {case_letter}: {output_file}")
        else:
            print(f"  ❌ No files processed for Case {case_letter}")

    print(f"\nTotal cells containing 'COUNTERPARTY_NOT_STATED': {before_count}")
...
def process_transaction_columns():
    ...
    case_files = list(input_dir.glob('Case_[A-E]_transaction_records.csv'))
    ...
    for case_file in case_files:
        print(f"\nProcessing columns for: {case_file.name}")
        ...
        if fraud_col and fraud_col in df.columns:
            ...
            print(f"  Sorted by {fraud_col}")
        else:
            print(f"  No fraud payment column found - no sorting applied")

        df.to_csv(case_file, index=False)
        print(f"  ✅ Processed {case_file.name}")
...
def merge_cd_transactions():
    ...
    for case_file in case_files:
        print(f"\nMerging C-D transactions for: {case_file.name}")
        df = pd.read_csv(case_file, dtype=str)
        df = trim_dataframe(df)
